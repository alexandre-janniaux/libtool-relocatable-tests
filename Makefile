...
lib.la: lib.c libexternal.a
	...
	$(LTLINK) $(CXX) -static -export-symbols-regex "^vlc_entry" -o $@ \
		-Wl,--whole-archive .deps/lib.lo -Wl,--no-whole-archive libexternal.a #-L. -lexternal

libmain_lib.la: main_lib.c lib.la
	mkdir -p .deps/
	$(LTCOMPILE) $(CC) -c -fPIC -static main_lib.c -o .deps/main_lib.lo
	$(LTLINK) $(CXX) -static -export-symbols-regex "^vlc_entry|^entrypoint" -o .deps/$@.o \
	...
	$(LTLINK) $(CXX) -static -o libmain_lib.la .deps/$@.lo \
		lib.la  -rpath /usr/local/lib

exe: libmain_lib.la stats
	$(LTCOMPILE) $(CC) -c -o .deps/main.lo main.c
	$(LTLINK) $(CXX) -o $@ .deps/main.lo libmain_lib.la
